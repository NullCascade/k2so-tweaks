...
function patch.on_data_updates()
	...
	if (data.raw["item"]["aluminum-plate"]) then
		...
		util.recipe.set_standardized_icon("processing-unit-via-aluminum", "processing-unit", "aluminum-plate")
	...
	if (data.raw["item"]["silicon-cell"]) then
		...
		util.recipe.set_standardized_icon("processing-unit-via-silicon-cell", "processing-unit", "silicon-cell")
